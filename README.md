...
| IPC `wlp4s0` | DHCP | 인터넷 |
| 로봇 AP | `192.168.50.1` (`EV-DL_AP`, **ch1**) | **무선 폴백 경로** + 개발 PC 직접 접속 |
...
- 🔴 **AP 채널은 스캔해서 고른다.** ch6 에서 주행 중 명령이 수백~수천 ms 밀려 로봇이 반복 정지했다(2026-09-03). 원인은 **인접 채널 간섭** — ch8 의 남의 AP 가 우리보다 강했고 ch10 에 89% 4개가 있었다. **ch1 로 옮기자 같은 신호에서 RTT 평균 75.8 → 3.0 ms**. 대역·워치독·큐 우선순위를 만지는 것보다 이게 압도적으로 효과가 컸다.
  ```bash
  nmcli dev wifi list                      # 채널별 혼잡도 먼저 본다
  ```
  ⚠️ AP 변경은 **모든 무선 접속을 끊는다** → **유선을 먼저 확보**하고 IPC 에서 유선 경유로 분리 실행할 것.
...
8. 🔴 **영상이 자꾸 멈추면 복구 장치를 먼저 의심한다** (2026-09-03). 무선에서 15초마다 화면이 얼었는데 로봇은 정상적으로 20.3 fps 를 보내고 있었다. 범인은 내가 만든 복구 장치 **둘**이었다: ① `vidserver.sendAll` 이 20 ms 백프레셔에서 포기하고 **주석과 달리 클라이언트를 끊었다** ② IPC 영상 워치독(4초)이 무선 멈춤마다 파이프라인을 재시작해 **키프레임 대기로 검은 화면을 늘렸다.** 고침: 보내기 전에 `SIOCOUTQ` 로 적체를 보고 **프레임을 건너뛰되 연결은 유지**(중간에 끊긴 프레임은 헤더+다음 키프레임으로 재동기), 진짜 끊김(EPIPE)만 닫는다. 워치독은 12초로. **결과: 재시작 6회/100초 → 0회/120초.**
1. 🔴 **영상이 검게 멈추면 좀비 TCP 연결이다.** navi 가 재시작되거나 무선이 튀면 로봇 쪽 소켓만 사라지고 IPC 쪽은 `ESTABLISHED` 로 남는다(half-open). 데이터만 끊기는데 **GStreamer 는 오류를 안 내서** ERROR/EOS 기반 재접속이 영영 안 돈다. 진단은 `ss -tni "dst <robot>:5000"` 의 `bytes_received` 를 두 번 재보면 끝 — 안 늘면 좀비다. 콘솔에 **영상 워치독**(4초 무프레임 → 파이프라인 재시작)을 넣어 자동 복구된다.
