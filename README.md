...
거리센서로 지나간 차량 수를 세서 화면 우측에 크게 띄운다 — **세 줄**(아이콘 탈리 / 대수 / 상태), 차 밑이면 **초록**·차 사이면 노랑:
...
첫 줄은 대수만큼 🚗 를 늘어놓지만 `MAX_ICONS`(10)에서 끊고 `+N` 으로 잇는다 — **숫자가 정답, 아이콘은 보조**. 0대여도 첫 줄을 비워 **세 줄을 유지**한다(줄 수가 바뀌면 하단 바 높이가 들썩여 영상 영역이 흔들린다).
...
| 오른쪽 스틱 ←→ | 좌 / 우회전 (비례) |
| D패드 ↑↓ | 주행 최대속도 ±0.01 m/s (0.02 ~ **0.05**, 시작 0.0239 = 3 RPM) |
| D패드 →← | 회전 최대속도 ±2 °/s (2 ~ **6**, 시작 2) |
