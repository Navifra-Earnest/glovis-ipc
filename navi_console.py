...
# 0.2 → 0.6 m (2026-09-08). 0.2 는 **차 하부에 딱 붙은** 값이라 배기 터널·서스펜션
# 처럼 트인 구간에서 "차 사이" 로 튀어 같은 차를 두 번 셀 수 있었다. 게다가 TF-Luna
# 측정범위가 0.2~8 m 라 판정 경계가 센서 규격 최소에 놓여 있었다(차 밑 실측 5~15 cm 는
# 규격 아래다). 차 사이는 **위가 트여 반사가 없어 무효**로 오거나 천장이라 2 m 넘으므로
# 0.2~2.0 m 사이 어디든 임계가 될 수 있다 → 아래쪽 끝에 붙일 이유가 없다.
# 올려도 차 사이 판정은 안 망가진다 — 판정식이 `valid 하고 임계 이하` 라서 무효는
# 임계와 무관하게 차 사이다.
# ⚠️ 차 사이인데도 차 밑으로 세면 내리면 된다: --under-cm 20
UNDER_CM, UNDER_HOLD_S, GAP_HOLD_S = 60.0, 2.0, 0.5
...
def selftest():
    ...
    # 임계는 차 밑(5~15cm)과 차 사이(무효 또는 2m 이상) 사이에 있어야 한다.
    # 아래쪽 끝(0.2m = 센서 규격 최소)에 붙이지 않는다 — 차 하부가 트이면 튄다.
    assert 30.0 <= UNDER_CM <= 150.0, f"임계가 판별 구간을 벗어났다: {UNDER_CM}cm"
